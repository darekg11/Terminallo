...
const importTerminals = path => async dispatch => {
  ...
  if (path) {
    try {
      ...
      const mappedTerminalInstances = TerminalService.importTerminalsToObject(
        loadedTerminalFile
      );
      dispatch(
        batchActions([
          TerminalActions.importTerminalInstances(
            mappedTerminalInstances.terminals
          ),
      ...
      dispatch(
        SpinnerActions.showSpinnerError(
          `Could not import terminals :( Error: ${err.message}`
        )
      );
    }
  } else {
...
const exportTerminals = path => async (dispatch, getState) => {
  ...
  if (path) {
    try {
      ...
      const terminalExportObject = TerminalService.exportTermninalsToObject(
        terminals
      );
      ...
      dispatch(
        SpinnerActions.showSpinnerError(
          `Could not export terminals :( Error: ${err.message}`
        )
      );
    }
  } else {
...
export default { exportTerminals, importTerminals, setTerminalsSourcePath };
